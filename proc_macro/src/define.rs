...
pub fn define(tts: TokenStream) -> TokenStream {
    ...
    impl ResolvedAlias {
        fn new(
            ...
            let alias_name_span = alias.span();
            ...
            dummy_use_stmts.insert_ident(alias);
    ...
    for (alias_name, (alias_name_span, entities)) in &nested_alias_map {
        ...
        flat_alias_map.insert(
            ...
            ResolvedAlias::new(
                &nested_alias_map,
                &mut dummy_use_stmts,
                Ident::new(alias_name, *alias_name_span),
                entities,
            ),
        );
    }
    ...
    flat_alias_map
        ...
        .chain(TokenStream::from_iter([
            ...
            TokenTree::Punct(Punct::new('=', Spacing::Joint)),
            TokenTree::Group(Group::new(Delimiter::Brace, dummy_use_stmts.0)),
            TokenTree::Punct(Punct::new(';', Spacing::Joint)),
        ]))
        .collect()
}
